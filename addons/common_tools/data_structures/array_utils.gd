...
static func sort_on_key(a_array: Array, a_key: Callable) -> Array:
...
## Get the index of the array for which the callable returns the maximum value
static func arg_max(a_array: Array, a_key: Callable) -> int:
	var ret: int = 0
	var max = a_key.call(a_array[0])
...
static func mean(a_array: Array) -> float:
	return sum(a_array)*1.0/a_array.size()
#endregion
